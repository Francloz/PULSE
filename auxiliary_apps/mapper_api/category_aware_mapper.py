...
filtered_concept_df = concept_df[
    ...
    (concept_df["vocabulary_id"].isin(["SNOMED", "CDM", "LOINC", "GENDER", "UCUM", "Ethnicity", "Race", "LOINC", 'ATC', "OMOP Extension"]))
][["concept_id", "concept_code", "concept_name", "vocabulary_id"]]
print(concept_df["vocabulary_id"].unique())
print(concept_df["domain_id"].unique())
ALL_CONCEPTS = filtered_concept_df

class BioBERTEntityLinker:
    def __init__(self, model_name="dmis-lab/biobert-base-cased-v1.1", top_k=5, debug=False, index_directory="./data/index/"):
        """
        ...
        for category in relevant_domains:
            ...
            if os.path.exists(path):
                print(f"Already built FAISS index for {category}")
                concepts_in_domain = concept_df[concept_df['domain_id'].isin(relevant_domains)]
                for concept_id, concept_name in zip(concepts_in_domain['concept_id'], concepts_in_domain['concept_name']):
                    self.concepts[category].append((concept_id, concept_name))

                self.index[category] = faiss.read_index(path)
                print(f"Cache FAISS {category} database loaded")
            else:
                print(f"Missing FAISS index for {category}")
                self.build_index()
                return

    def _get_concept(self, concept_id):
        defs = ALL_CONCEPTS[ALL_CONCEPTS["concept_id"] == concept_id]
        return defs
    ...
    def build_index(self):
        ...
        for category in relevant_domains:
            concepts_in_domain = concept_df[concept_df['domain_id'] == category]
        ...
        for category in relevant_domains:
            ...
            for text_batch, cuid_batch in tqdm(zip(tok_batches, cuid_batches), total=len(tok_batches), desc=f"Processing {category} concepts", unit="batch"):
                ...
                self.concepts[category].extend(zip(cuid_batch, text_batch))
    ...
    def link(self, text: str, category: str):
        ...
        for i in range(self.top_k):
            ...
            all_info = self._get_concept(cui)
            filtered_info = all_info[important_fields].astype(object)

            row_dict = OrderedDict()
            row_dict["score"] = float(score)
            row_dict.update(filtered_info.to_dict())
